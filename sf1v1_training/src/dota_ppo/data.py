...
CURRENT_LOCAL_REWARD_VERSION = "sf1v1_passive_v1"
CURRENT_LOCAL_OBSERVATION_VERSION = "sf1v1_passive_v1"
...
def require_training_collection_role(path: Path) -> dict[str, object]:
    """Keep held-out evaluation archives out of PPO updates."""
    metadata = load_rollout_metadata(path)
    role = metadata.get("collection_role")
    if role != "training":
        raise ValueError(
            f"{path} has collection_role {role!r}; PPO accepts only explicitly collected training archives"
        )
    return metadata


def inspect_rollout_candidates(paths: list[Path]) -> dict[str, object]:
    """Explain PPO readiness without mutating archives or training a model."""
    records: list[dict[str, object]] = []
    for path in paths:
        record: dict[str, object] = {"path": str(path), "ppo_eligible": False}
        try:
            rollout = load_rollouts(path)
            metadata = load_rollout_metadata(path)
            reasons: list[str] = []
            if rollout.source != "local_instrumented_lobby": reasons.append("source is not local_instrumented_lobby")
            if metadata.get("reward_version") != CURRENT_LOCAL_REWARD_VERSION: reasons.append(
                f"reward_version is {metadata.get('reward_version')!r}, expected {CURRENT_LOCAL_REWARD_VERSION!r}")
            if metadata.get("observation_version") != CURRENT_LOCAL_OBSERVATION_VERSION: reasons.append(
                f"observation_version is {metadata.get('observation_version')!r}, expected {CURRENT_LOCAL_OBSERVATION_VERSION!r}")
            if metadata.get("collection_role") != "training": reasons.append(
                "collection_role is not 'training' (evaluation archives are held out)")
            if rollout.action_masks is None: reasons.append("action masks are missing")
            if not bool(rollout.dones[-1]): reasons.append("archive ends mid-episode")
            record.update({"steps": len(rollout.actions), "completed_episodes": int(rollout.dones.sum()),
                           "policy_checkpoint_sha256": metadata.get("policy_checkpoint_sha256"),
                           "collection_role": metadata.get("collection_role"), "reasons": reasons,
                           "ppo_eligible": not reasons})
        except (OSError, ValueError, KeyError, json.JSONDecodeError) as error:
            record["reasons"] = [str(error)]
        records.append(record)
    return {"expected_reward_version": CURRENT_LOCAL_REWARD_VERSION,
            "expected_observation_version": CURRENT_LOCAL_OBSERVATION_VERSION,
            "archives": records, "eligible_archives": sum(bool(item["ppo_eligible"]) for item in records)}
...
def merge_rollouts(paths: list[Path], output: Path) -> dict[str, object]:
    ...
    for path in paths:
        ...
        require_training_collection_role(path)
