...
class PolicyBridge:
    ...
    def __init__(self, checkpoint: Path, rollout_output: Path, device_name: str = "cuda", human_orders: Path | None = None,
                 calibration_output: Path | None = None, collection_role: str = "training") -> None:
        ...
        if collection_role not in {"training", "evaluation"}:
            raise ValueError("collection_role must be 'training' or 'evaluation'")
    ...
    def act(self, request: dict[str, Any]) -> dict[str, object]:
        ...
        observation_version = str(request.get("observation_version", OBSERVATION_VERSION))
        if observation_version != OBSERVATION_VERSION:
            raise ValueError(
                f"observation_version must be {OBSERVATION_VERSION!r}, got {observation_version!r}"
            )
        ...
        with self.lock:
            self.pending[decision_id] = PendingDecision(
                observation, mask, action, log_prob, value, game_time, reward_version, environment_json
            )
    ...
    def flush(self, request: dict[str, Any] | None = None) -> dict[str, object]:
        ...
        with self.lock:
            ...
            environments = {row.environment_json for row in decisions}
            if len(environments) != 1:
                raise ValueError("cannot save a rollout containing mixed environment metadata")
            data = Rollouts(
...
def serve_bridge(checkpoint: Path, rollout_output: Path, *, port: int = 8765, device_name: str = "cuda", human_orders: Path | None = None,
                 calibration_output: Path | None = None, collection_role: str = "training") -> None:
    bridge = PolicyBridge(checkpoint, rollout_output, device_name, human_orders, calibration_output, collection_role)
