...
from .data import (inspect_rollout_candidates, load_rollouts, load_trajectories, merge_rollouts,
                   require_current_local_observation_version, require_current_local_reward_version,
                   require_policy_checkpoint, require_training_collection_role)
...
def parser() -> argparse.ArgumentParser:
    ...
    inspect = commands.add_parser("inspect-rollouts", help="explain which local archives are eligible for PPO")
    inspect.add_argument("paths", type=Path, nargs="+", help=".npz files or directories to scan recursively")
    ...
    bridge.add_argument("checkpoint", type=Path); bridge.add_argument("--rollouts", type=Path, required=True); bridge.add_argument("--port", type=int, default=8765); bridge.add_argument("--device", default="cuda"); bridge.add_argument("--human-orders", type=Path); bridge.add_argument("--calibration", type=Path); bridge.add_argument("--collection-role", choices=("training", "evaluation"), default="training")
    ...
    supervise.add_argument("--rollout-dir", type=Path, default=Path("data/training")); supervise.add_argument("--checkpoint", type=Path, required=True); supervise.add_argument("--run-dir", type=Path, required=True)
...
def main() -> int:
    ...
    if args.command == "parse":
    ...
    elif args.command == "inspect-rollouts":
        paths = [item for path in args.paths for item in ([path] if path.is_file() else sorted(path.rglob("*.npz")))]
        print(inspect_rollout_candidates(paths))
        ...
        require_training_collection_role(args.rollouts)
        ...
        serve_bridge(args.checkpoint, args.rollouts, port=args.port, device_name=args.device, human_orders=args.human_orders,
                     calibration_output=args.calibration, collection_role=args.collection_role)
