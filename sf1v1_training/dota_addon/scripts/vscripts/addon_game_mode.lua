...
function GameMode:InitGameMode()
    ...
    Convars:RegisterCommand("rl_ppo_opponent", function()
        if GameRules.GameMode ~= nil and GameRules.GameMode.rl_scenario ~= nil then
            GameRules.GameMode.rl_scenario:PrintOpponent()
        end
    end, "Local-only: print the passive Shadow Fiend opponent state", 0)
...
function GameMode:RestartRLPPO()
    ...
    if self.rl_scenario ~= nil then self.rl_scenario:Destroy() end
    self.rl_bridge = RLPPOBridge()
    self.rl_scenario = RLLaneScenario()
