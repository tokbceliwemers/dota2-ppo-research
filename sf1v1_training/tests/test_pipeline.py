...
from dota_ppo.data import (CURRENT_LOCAL_OBSERVATION_VERSION, CURRENT_LOCAL_REWARD_VERSION,
                           Rollouts, Trajectories, checkpoint_sha256, inspect_rollout_candidates,
                           load_rollout_metadata, load_rollouts, load_trajectories, merge_rollouts,
                           require_training_collection_role, save_rollouts)
...
def test_passive_opponent_addon_enforces_fixed_shadow_fiend_progression() -> None:
    ...
    assert 'sf1v1_passive_v1' in bridge
    assert 'local OBSERVATION_DIM = 32' in bridge
    assert 'GetAttackTarget' in bridge
    assert 'GetLastAttackTime' in bridge
    assert 'hero_attack_cooldown = seconds_per_attack' in bridge
    assert 'PASSIVE_OPPONENT_NAME = "npc_dota_hero_nevermore"' in scenario
    assert 'SetOpponent(self.opponent, "passive", PASSIVE_OPPONENT_IDENTITY)' in scenario
    assert 'HOLD_POSITION' in scenario
...
def test_bridge_writes_separate_calibration_events(tmp_path: Path) -> None:
    ...
    event = {"schema_version": SCHEMA_VERSION, "event": "transition", "decision_id": "d1", "action_name": "attack",
             ...
             "ally_count": 4.0, "hero_attack_cooldown": 0.71, "attack_recovery": 0.25,
             "in_attack_range": 1.0, "last_hit": True, "hero_dead": False, "done": True}
    ...
    assert report["measured_lane_config"]["hero_attack_cooldown"] == 0.71
    assert "hero_attack_cooldown" not in report["unmeasured_lane_config"]
...
def test_bridge_persists_training_role_and_environment_metadata(tmp_path: Path) -> None:
    checkpoint = tmp_path / "policy.pt"
    behavior_clone(_replay_data(), ACTION_DIM, torch.device("cpu"), checkpoint, epochs=1, batch_size=4)
    output = tmp_path / "rollout.npz"
    bridge = PolicyBridge(checkpoint, output, "cpu", collection_role="training")
    bridge.act({"observation": [0.0] * OBSERVATION_DIM, "action_mask": [True] + [False] * (ACTION_DIM - 1),
                "environment": {"map": "template_map", "opponent_mode": "passive"}})
    bridge.flush({"final_reward": 0.0})
    metadata = load_rollout_metadata(output)
    assert metadata["collection_role"] == "training"
    assert metadata["environment"] == {"map": "template_map", "opponent_mode": "passive"}


def test_inspection_and_training_guard_reject_held_out_evaluation(tmp_path: Path) -> None:
    path = tmp_path / "evaluation.npz"
    rollout = Rollouts(np.zeros((2, OBSERVATION_DIM), np.float32), np.array([0, 0]), np.zeros(2, np.float32),
    ...
    save_rollouts(path, rollout, {"reward_version": CURRENT_LOCAL_REWARD_VERSION,
                                  "observation_version": CURRENT_LOCAL_OBSERVATION_VERSION,
                                  "collection_role": "evaluation", "policy_checkpoint_sha256": "held-out"})
    report = inspect_rollout_candidates([path])
    assert report["eligible_archives"] == 0
    assert "held out" in report["archives"][0]["reasons"][0]
    try:
        require_training_collection_role(path)
    except ValueError as error:
        assert "training" in str(error)
    else:
        raise AssertionError("evaluation archive was accepted for PPO")
