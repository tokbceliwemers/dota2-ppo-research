...
class DotaTerrainLaneEnvTest(unittest.TestCase):
    ...
    def test_calibration_adopts_only_directly_measured_attack_period(self) -> None:
        report = self.directory / "calibration.json"
        report.write_text(json.dumps({
            "source": "local_lane_calibration",
            "measured_lane_config": {"tick_seconds": 0.3, "hero_attack_range": 525,
                                       "hero_attack_cooldown": 0.71, "attack_range_buffer": 99},
        }), encoding="utf-8")
        config = TerrainLaneConfig.from_calibration(report)
        self.assertEqual(config.tick_seconds, 0.3)
        self.assertEqual(config.hero_attack_range, 525)
        self.assertEqual(config.hero_attack_cooldown, 0.71)
        self.assertEqual(config.attack_range_buffer, TerrainLaneConfig().attack_range_buffer)
