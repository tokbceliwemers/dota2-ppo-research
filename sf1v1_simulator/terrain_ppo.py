...
from dota_lane_env import ACTION_DIM, DotaTerrainLaneEnv, TerrainLaneConfig
...
def collect_rollout(model: ActorCritic, environments: int, horizon: int, data_directory: Path,
                    device: torch.device, seed: int, environment_config: TerrainLaneConfig) -> Rollouts:
    ...
    envs = [DotaTerrainLaneEnv(data_directory, environment_config) for _ in range(environments)]
...
def train_terrain_ppo(checkpoint_in: Path, checkpoint_out: Path, data_directory: Path, *, updates: int,
                      ...
                      device: torch.device, seed: int, calibration_report: Path | None = None) -> dict[str, object]:
    ...
    environment_config = (TerrainLaneConfig.from_calibration(calibration_report)
                          if calibration_report is not None else TerrainLaneConfig())
    ...
    _checkpoint(checkpoint_out, model, None, {
        ...
        "data_directory": str(data_directory), "lane_config": asdict(environment_config),
...
def main() -> int:
    ...
    parser.add_argument("--calibration-report", type=Path, help="optional local-lobby calibration JSON report")
